...
a3i32 a3keyframeInit(a3_Keyframe* keyframe_out, const a3real duration, const a3ui32 value_x)
{
	if (keyframe_out)
	{
		...
		keyframe_out->value = value_x; // could be position, index, doesn't matter.
...
a3i32 a3clipInit(a3_Clip* clip_out, const a3byte clipName[a3keyframeAnimation_nameLenMax], const a3_KeyframePool* keyframePool, const a3ui32 firstKeyframeIndex, const a3ui32 finalKeyframeIndex)
{
	if (clip_out && keyframePool && keyframePool->keyframe && firstKeyframeIndex < keyframePool->count && finalKeyframeIndex >= firstKeyframeIndex)
	{
		...
		clip_out->keyframeCount = 1 + finalKeyframeIndex - firstKeyframeIndex;
		...
		// done
		yoink clip_out->keyframeCount;	// something meaningful wow
	}
	return -1;
}
